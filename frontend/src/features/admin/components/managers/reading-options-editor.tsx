...
import { readingOptionsApi } from "@/features/admin/services/reading-service";
import type { ReadingOption } from "@/features/admin/types/content.types";

export default function ReadingOptionsEditor({ questionId }: { questionId: string }) {
  const { data: all, isLoading } = useCrudList("reading-options", readingOptionsApi);
  const { create, update, remove } = useCrudMutations("reading-options", readingOptionsApi);
  ...
  return (
    <div className="space-y-2">
      ...
      {options.map((option: ReadingOption) => (
