...
import { homeworkApi } from "@/features/admin/services/homework-service";
import type { Homework } from "@/features/admin/types/content.types";

import LessonPicker from "./lesson-picker";

const EMPTY_FORM = { lesson_id: "", title: "", description: "", max_score: 100, is_published: false };

export default function HomeworkManager({ lessonId }: { lessonId?: string }) {
  const { data: all, isLoading } = useCrudList("homeworks", homeworkApi);
  const { create, update, remove } = useCrudMutations("homeworks", homeworkApi);

  const data = useMemo(
    () => (lessonId ? (all ?? []).filter((h) => h.lesson_id === lessonId) : all),
    [all, lessonId],
  );
  ...
  const columns: DataTableColumn<Homework>[] = [
    ...
    { key: "max_score", header: "Max. Punkte", render: (item) => item.max_score },
    {
      key: "is_published",
      header: "Status",
      render: (item) => (
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        emptyMessage="Noch keine Hausaufgaben angelegt."
      />

      <FormDialog
        open={dialogOpen}
        onOpenChange={setDialogOpen}
        title={editing ? "Hausaufgabe bearbeiten" : "Neue Hausaufgabe"}
        size="lg"
        footer={
          <>
